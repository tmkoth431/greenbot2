...
module.exports = {
  ...
  async execute(int, c) {
    ...
    if (user.combat) {
      embededd.setDescription('You cannot level up while in combat!').setThumbnail('https://i.imgur.com/IRh7QZo.png').setColor('#ff0000')
      return int.reply({ embeds: [embededd] })
    }
    if (user.level_points <= 0) {
      embededd.setDescription('You do not have any level points!').setThumbnail('https://i.imgur.com/IRh7QZo.png').setColor('#ff0000')
      return int.reply({ embeds: [embededd] })
    }
    if (amount == 'max') amount = user.level_points
    if (isNaN(amount)) {
      embededd.setDescription('Please enter a number!').setThumbnail('https://i.imgur.com/IRh7QZo.png').setColor('#ff0000')
      return int.reply({ embeds: [embededd] })
    }
