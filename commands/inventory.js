...
module.exports = {
  ...
  async execute(int, c) {
    ...
    if (items.filter(a => a.type == 'w' && a.amount > 0).map(item => `${item.item_id}`).join('\n')) wtext = `${ctext ? '\n\n' : ''}Weapons:\n` + items.sort((a, b) => a.shop_id - b.shop_id).filter(a => a.type === 'w' && a.amount > 0).map(item => `[ID: ${item.shop_id}] ${item.amount} ${item.amount >1? `${item.item_id}` + 's' : `${$item.item_id}`} ${item.item_id} Damage: ${item.damage} ${item.attribute != '' ? `Attribute: ${item.attribute}` : ''} ${!item.enchant ? '' : `Enchant: ${item.enchant}`}`).join('\n')
