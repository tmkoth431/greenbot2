...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('inventory')
    .setDescription('veiws inventory')
  ...
  async execute(int, c) {
    ...
    if (!user) return int.reply(`unnable to find ${target}`)
    const items = await user.getItems()
    if (!user) return int.reply(`${target} does not exist`)
    if (!items.length) return int.reply(`${target.tag} has nothing!`);
    let wep
    ...
    return int.reply(Formatters.codeBlock('' + ctext + wtext + etext + wep));
  },
}