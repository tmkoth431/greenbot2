  ...
  async execute(int, userEffects, tUserEffects, user, tUser) {
    ...
    switch (rand) {
      ...
      case 4:
        ...
        tUserEffects.save()
        int.channel.send(`<@${tUser.user_id}>, you have been set on fire`)
        break
      case 5:
        tUserEffects.poison = Number(5)
        tUserEffects.save()
        int.channel.send(`<@${tUser.user_id}>, you have been poisoned`)
        break
      case 6:
        tUser.fish_exp += Math.round(tUser.fish_exp / 15)
        tUser.save()
        int.channel.send(`<@${tUser.user_id}>, you got better at fishing`)
        break
      case 7:
        tUser.luck += Number(2)
        tUser.save()
        int.channel.send(`<@${tUser.user_id}>, you got luckier`)
        break
      case 8:
        tUser.curse = true
        tUser.curse_time = Date.now()
        tUser.save()
        int.channel.send(`<@${tUser.user_id}>, you have been cursed`)
        break
    }
  },
}